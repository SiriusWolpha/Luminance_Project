...
status_t LPI2C_MasterStart(LPI2C_Type *base, uint8_t address, lpi2c_direction_t dir)
{
    ...
    if (kStatus_Success == result)
    {
        ...
        printf("Func:LPI2C_MasterStart:ClearStatusFlags done\r\n");
        /* Turn off auto-stop option. */
        base->MCFGR1 &= ~LPI2C_MCFGR1_AUTOSTOP_MASK;

        /* Wait until there is room in the fifo. */
        result = LPI2C_MasterWaitForTxReady(base);
        printf("Func:LPI2C_MasterStart:TxFIFO ready");
        if (kStatus_Success == result)
        {
            printf(" reval = kStatus_Success\r\n");
            /* Issue start command. */
...
{
    uint32_t loopcnt = 0;//用于LPI2C_MasterStop内部循环
    /* Wait until there is room in the fifo. */
    ...
    if (kStatus_Success == result)
    {
        ...
        LPI2C_Type *rev = LPI2C1;
        printf("send StopCmd to MTDR:MTDR=[nothing]\r\n");//,rev->MTDR);
        ...
        while (result == kStatus_Success)
        {
            ...
            uint32_t status = LPI2C_MasterGetStatusFlags(base);
            LPI2C_Type *rev = LPI2C1;
            printf("loop%d Judging Flag:MSR = [nothing here]\r\n",loopcnt);//,rev->MSR);
            ...
            if(loopcnt>=100){
                printf("MSR error,still in loop\r\n");
                break;
            }
...
status_t LPI2C_MasterReceive(LPI2C_Type *base, void *rxBuff, size_t rxSize)
{
    ...
    if (rxSize != 0U)
    {
        ...
        result = 0;//LPI2C_MasterWaitForTxReady(base);
        if (kStatus_Success == result)
        {
...
status_t LPI2C_MasterSend(LPI2C_Type *base, void *txBuff, size_t txSize)
{
    ...
    {
    	LPI2C_Type *rev = LPI2C1;
        //->>printf("now MTDR=%x ",rev->MTDR);
        /* Wait until there is room in the fifo. This also checks for errors. */
        result = LPI2C_MasterWaitForTxReady(base);
        LPI2C_Type *rev2 = LPI2C1;
        //->>printf("after fifo send:MTDR=%x ",rev2->MTDR);
        if (kStatus_Success != result)
        {
            break;
        }
        ...
        base->MTDR = *buf++;
        LPI2C_Type *rev3 = LPI2C1;
        //->>printf("after buf write:MTDR=%x \r\n",rev3->MTDR);
    }
                                         ...
                                         lpi2c_master_transfer_t *transfer)
{
    ...
    {
    	printf("Error : kStatus_InvalidArgument\n");
        return kStatus_InvalidArgument;
    }
    ...
    {
    	printf("Error : kStatus_LPI2C_Busy\n");
